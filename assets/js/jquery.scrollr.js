    ...
    var methods = {
        init: function (options) {
            return this.each(function () {
                var self = $(this).addClass('scrollr-self'),
                    data = self.data('scrollr-self');
                if (!data) {
                    // Defaults
                    var opts = $.extend({
					...
					if (parseInt(opts.innerHeight, 10) < parseInt(opts.viewPort.height(), 10)) {
                       self.data('scrollr-self', opts); //pass data for other functions
	                   methods.destroy.apply(self);
                        ...
                        }).append(opts.handle).appendTo(opts.viewPort);
                ...
                function handle() {
                    ...
                    opts.handle.bind('mousedown.scrollr', function (e) {
                        e.preventDefault();
                        opts.handle.mousePageY = e.pageY; 
                        opts.handle.dragPos = opts.handle.dragger.position().top;
/*
                        opts.handle.leftPos;
                        opts.handle.topPos;
*/
